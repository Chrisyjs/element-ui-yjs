...
Notification.close = function(id, userOnClose) {
  ...
  const removedHeight = instance.isStack ? -16 : instance.dom.offsetHeight;
  for (let i = index; i < len - 1 ; i++) {
    if (instances[i].position === position) {
      instances[i].dom.style[instance.verticalProperty] = instances[i].isStack ? instances[i].verticalOffset + 'px'
