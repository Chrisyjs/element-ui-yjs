...
export default {
  ...
  methods: {
    ...
    clearSelectedNode() {
      this.selectedData = '';
      this.$refs.tree.getCurrentKey() && this.$refs.tree.setCurrentKey(null);
    },
    // 多选，清空所有勾选
    clearSelectedNodes() {
    ...
    handleCheckChange() {
      ...
      switch (this.valueConsistsOf) {
        case 'ALL': // 所有选中节点，不包括半选节点
          checkedKeys = checkedKeys;
          break;
        case 'LEAF_PRIORITY': // 叶子节点优先
          checkedKeys = checkedNodes
            .map(item => {
              return !item.childrenList && item.menuId;
            })
            .filter(item => item);
          break;
        case 'BRANCH_PRIORITY': // 选中父节点优先
          checkedKeys = checkedNodes
            .map(item => {
              return (
                !this.$refs.tree.getNode(item.menuId).parent.checked &&
                item.menuId
              );
            })
            .filter(item => item);
          break;
        case 'ALL_WITH_INDETERMINATE': // 所有节点，包括半选节点
          checkedKeys = checkedKeys
