...
:::demo 将`isStack`属性设置为`true`即可堆叠。
```html
<template>
  <el-button
    plain
...
:::demo 将`showConfirmButton`属性设置为`true`即可显示按钮，同时需要按钮文案`confirmButtonText`，也可设置点按钮时的回调`onConfirm`。
```html
<template>
  <el-button
    plain
...
| showConfirmButton | 是否显示按钮 | boolean | - | false |
| confirmButtonText | 按钮文案 | string | - | - |
| onConfirm | 点击按钮时的回调函数 | function | - | - |
...
| closeAll | 关闭所有的 Notification |
| clear | 清空当前的 Notification（不执行close方法） |
| clearAll | 清空所有的 Notification（不执行close方法） |
